...
def getArgs():
    ...
    parse.add_argument('--arch', '-a', metavar='ARCH', default='TUNet',
                       help='UNet/resnet34_unet/unet++/myChannelUnet/Attention_UNet/segnet/r2unet/fcn32s/fcn8s/seunet')
    parse.add_argument("--batch_size", type=int, default=2)
    parse.add_argument('--dataset', default='tongue',
                       help='dataset name:tongue_dateset')
...
def getModel(args):
    ...
    if args.arch == 'resnet34_unet':
        model = resnet34_unet(1,pretrained=False)
    if args.arch == 'unet++':
        args.deepsupervision = True
        model = NestedUNet(args,3,1)
...
def getDataset(args):
    train_dataset = TongueDataset(r'train', transform=x_transforms, target_transform=y_transforms)
    train_dataloaders = DataLoader(train_dataset, batch_size=args.batch_size)
    val_dataset = TongueDataset(r"val", transform=x_transforms, target_transform=y_transforms)
    val_dataloaders = DataLoader(val_dataset, batch_size=1)
    test_dataset = TongueDataset(r"test", transform=x_transforms, target_transform=y_transforms)
...
def mytest(val_dataloaders,save_predict=False):
...
if __name__ =="__main__":
    ...
    if 'test' in args.action:
        mytest(test_dataloaders, save_predict=True)
